...
|-------|------------------|---------|
| Plaintext paste content | Server compromise, log exposure | Client-side E2EE **on by default** for new pastes (XSalsa20-Poly1305 via NaCl `secretbox`; **Argon2id** 19 MiB/t=2 for password mode in version 4, legacy v≤3 pastes use PBKDF2-SHA-256 300k; key in URL fragment for key mode). Plaintext is **length-padded** to 256-byte buckets before encryption so ciphertext length doesn't reveal the exact secret size. Opt out per paste via Encryption → "None". |
...
- **Encryption versions** — `0` = plaintext; `2` = legacy E2EE (content only, plaintext title, PBKDF2, unpadded); `3` = E2EE content + title (PBKDF2, unpadded); `4` = E2EE content + title, **Argon2id** password mode, **length-padded** (the default for new encrypted pastes). The KDF and whether to strip padding are selected on decrypt by the paste's `version`, so legacy pastes keep decrypting forever. The KDF dispatch + pad/unpad live in `crypto-shared.ts` as a single source of truth shared by the main thread and the Web Worker. The title is an independent `secretbox` blob under the same key/salt; `CodeViewer` decrypts it best-effort after the content ("🔒 Encrypted paste" placeholder on failure / pre-decrypt), and `PasteViewer` shows a neutral `document.title` until decrypted so the ciphertext title never lands in the tab title or history.
- **Argon2id rationale** — password-mode pastes can be publicly listed, so anyone can fetch the ciphertext and brute-force the password **offline** on GPUs, where PBKDF2-SHA-256 is weakest. Argon2id is memory-hard (19 MiB) and the current OWASP first choice, raising the offline-attack cost by orders of magnitude. Runs in-browser/worker/node via `hash-wasm` (WASM base64-inlined into the bundle — no separate fetch).
