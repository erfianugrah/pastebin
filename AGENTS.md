...
| Path | What | Install |
|------|------|---------|
| `/` (root) | CF Worker backend (Hono router, DDD layers) | `bun install` |
| `astro/` | Static frontend (Astro + React + Tailwind v4 + shadcn/ui) | `cd astro && bun install` |
...
- **Realtime relay** in `src/infrastructure/realtime/`: `recentFeedDurableObject.ts` (`RecentFeedDO`), `phoenixClient.ts` (hand-rolled Phoenix v1.0.0), `feedHub.ts` (fan-out), `contract.ts` (frame types)
- **Bench harness** in `bench/`: `worker.ts` - standalone module Worker (`pg`, `postgres`, `jose`) with `/reads` `/writes` `/auth` `/branch` endpoints, not wired into the app; documents the CF<->Supabase latency matrix. Run with `wrangler dev --remote --config bench/wrangler.jsonc`
- **CI** in `.github/workflows/`: `ci.yml` (typecheck/test/lint/build), `deploy.yml` (build -> deploy -> smoke -> RLS), `preview.yml` + `preview-teardown.yml` (per-PR Supabase preview branch -> Worker preview version)
...
- All schema in `supabase/migrations/` - 23 files, applied to `dewddkcmwrzbpynylyhg`
...
- Realtime: live `/recent` feed via a **Durable Object relay** (reinstated in `20260714120000_reinstate_realtime_recent_feed.sql`). An `AFTER INSERT` trigger on public pastes calls `realtime.send('recent:public', 'paste_created', <safe metadata>)`; a single `RecentFeedDO` (`src/infrastructure/realtime/`) holds ONE server-side Supabase Realtime subscription and fans out to browsers over a same-origin `/api/recent/live` WebSocket. The browser never opens a socket to `*.supabase.co`, so CSP stays `connect-src 'self'` and the anon key stays server-side (the BFF invariant the earlier drop protected). The Phoenix v1.0.0 protocol is hand-rolled (no `realtime-js`); Broadcast `replay` on rejoin closes the reconnect gap; `RecentPastes.tsx` dedups by id and keeps `GET /api/recent` (15s) as a poll fallback. Plan + verification: `docs/plans/2026-07-14-realtime-recent-do.md`. The DO's outbound WS uses `fetch(https://...)` (CF rejects `wss://` in `fetch`), and `securityHeaders` skips 101 responses (else the `webSocket` property is dropped).
...
- **Explicit GRANTs are MANDATORY for every new `public.*` table** (rule applies after Supabase's Oct 30, 2026 cutover — see [Supabase changelog 45329](https://supabase.com/changelog/45329-breaking-change-tables-not-exposed-to-data-and-graphql-api-automatically)). Existing tables (`pastes`, `slugs`) keep their grants; this rule only constrains NEW migrations. The Worker uses `service_role` so that grant is the minimum required:
  ...
  ```
  `bun run check:migrations` (CI) greps for `CREATE TABLE public.` without a matching `GRANT … TO service_role` in the same file and fails the build.

## Commands

```bash
# Dev
bun run dev:all          # Astro UI (port 3000) + Worker (port 8787) concurrently
bun run dev:ui           # Astro only
bun run dev              # Worker only (wrangler dev)

# Build — UI must build first (outputs to astro/dist/)
bun run build:ui         # Astro build (also runs update-prism)
bun run build            # build:ui + wrangler build

# Deploy
bun run deploy           # build:ui + wrangler deploy
bun run deploy:prod      # build:ui + wrangler deploy --env production

# Quality
bun run check            # tsc --noEmit (root tsconfig only)
bun run lint             # eslint — src/**/*.ts only, not astro code
bun run test             # vitest run (src/tests/** + astro/src/lib/**)
bun run test:ui          # cd astro && bunx vitest run (component tests, jsdom)
bun run test:e2e         # playwright — runs against PRODUCTION (paste.erfi.io)
bun run test:smoke       # tsx scripts/smoke-test.ts — live API + Supabase verification
bun run test:race        # tsx scripts/concurrent-burn-test.ts — concurrent burn race-free check
bun run test:rls         # tsx scripts/verify-rls.ts — Supabase Auth + RLS end-to-end (2 test users)
bun run test:all-live    # runs all 3 live suites in sequence with cooldowns
bun run test:all         # test + test:ui + test:e2e
...
bun run test:smoke:tail
bun run test:race:tail
bun run test:rls:tail
bun run test:all-live:tail
bun run test:e2e:tail
...
- **Two install steps**: Root `bun install` does NOT install `astro/` deps. Run both.
...
- **Vitest split**: Root vitest includes `src/tests/**` + `astro/src/lib/**`. Component tests (`astro/src/components/**`) require running separately via `bun run test:ui`.
