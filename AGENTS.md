...
- **Encryption versions** (DB `version` int): `0` = plaintext, `2` = legacy E2EE (content only, plaintext title), `3` = E2EE content **and** title. New encrypted pastes are version 3 and default-on (key-mode). The title is encrypted client-side as an independent `secretbox` blob under the same key/salt as the content; `CodeViewer` decrypts it after the content and shows a "🔒 Encrypted paste" placeholder until then. The `GetRecentPastesQuery` / `SearchPastesQuery` DTOs withhold `title`+`language` (send `null`) and add `isEncrypted` for any encrypted paste, so the public listing never carries ciphertext or metadata. Editing a paste never force-encrypts (the update path leaves `version`/`is_encrypted` untouched).
