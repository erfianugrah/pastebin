 ...
 */
async function encryptData(data: string, keyBase64: string, isPasswordDerived = false, saltBase64?: string): Promise<string> {
	try {
	...
	try {
		...
		switch (operation) {
			case 'deriveKey':
				...
				result = await deriveKeyFromPassword(params.password, params.salt, params.isLargeFile || false);
